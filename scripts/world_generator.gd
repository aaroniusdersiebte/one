...
signal world_completed(rewards)
signal world_failed
...
func configure_world(world_data):
	# Konfiguriere die Welt basierend auf den übergebenen Daten
	print("Konfiguriere Welt mit Daten: ", world_data)
	
	# Beispiel: Anzahl der Wellen anpassen
	if "waves" in world_data:
		# Setze Wellen-Anzahl in deinem Spawner
		var spawner = get_node_or_null("WaveSpawner")
		if spawner:
			# Annahme: Dein Spawner hat eine Eigenschaft wie enemies_per_wave_base
			spawner.enemies_per_wave_base = world_data.waves
	
	# Schwierigkeit anpassen
	if "difficulty" in world_data:
		var difficulty = world_data.difficulty
		# Hier die Schwierigkeit anpassen, z.B.
		var enemy_multiplier = [1.0, 1.5, 2.0][difficulty] 
		# Anwenden auf deinen Spawner oder andere Systeme
	
	# Spezielle Bedingungen verarbeiten
	if world_data.get("special") != null:
		var special = world_data.special
		match special.id:
			"fast_enemies":
				# Implementiere schnellere Gegner
				print("Aktiviere schnellere Gegner")
			"tough_enemies":
				# Implementiere härtere Gegner
				print("Aktiviere härtere Gegner")
